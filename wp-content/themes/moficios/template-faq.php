...
set_query_var('ENTRY', 'faq');
get_header();
$search = filter_input(INPUT_GET, 'search');
$questions = get_field('questions');
if ($search) {
	function filter_questions($q) {
		global $search;
		$question = strpos(strtolower($q['question']), strtolower($search));
		if ($question != false) {
			return true;
		}
		$answer = strpos(strtolower($q['answer']), $search);
		if ($answer != false) {return true;}
		return false;
	}
	$questions = array_filter($questions, 'filter_questions');
}
?>

<div id="faq" class="x-container shadow">
	<div class="row">
        <div class="col-12 hero">
            <h1>¿En qué podemos ayudarte?</h1>
            <form action="" class="search">
                <input type="search" name="search" placeholder="Escribes palabras de búsqueda" value="<?php echo $search ?>">
            </form>
            <p class="mt-3">Lorem imsp lorem lorem lorem sh lor</p>
        </div>
    </div>
    <?php if(count($questions) > 0): ?>
    <div class="list-questions row">
        <?php 
        $a = 1;
        ...
        <div class="question col-12">
            <div class="px-4 py-5">
                ...
                <div class="valoration-answer shadow">
                    <div class="front-answer">
                        <h3>¿Te ayudo está respuesta?</h3>
                        <div class="btns">
                            <a href="javascript:void(0)" class="btn yes" order="<?php echo $a;?>" value="<?php echo $question['valoration_si'] ?>">Si</a>
                            <a href="javascript:void(0)" class="btn btnWhite no" order="<?php echo $a;?>" value="<?php echo $question['valoration_no'] ?>">No</a>
                        </div>
                    </div>
                    <div class="back-answer">
                       <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 475.099 475.099" xml:space="preserve"><g>
    ...
    <?php else: ?>
    <div class="empty-results">No se encontraron resultados.</div>
    <?php endif; ?>
</div>
<?php get_footer();